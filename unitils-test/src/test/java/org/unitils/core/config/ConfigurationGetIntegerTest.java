 ...
 * @author Tim Ducheyne
 */
public class ConfigurationGetIntegerTest {
    ...
    public void validValue() {
        Integer result = configuration.getInteger("intProperty");
        assertReflectionEquals(5, result);
    }

    @Test(expected = UnitilsException.class)
    public void invalidValue() {
        configuration.getInteger("invalidIntValue");
    }

    @Test
    public void valueIsTrimmed() {
        Integer result = configuration.getInteger("valueWithSpaces");
        assertReflectionEquals(5, result);
    }

    @Test(expected = UnitilsException.class)
    public void notFound() {
        configuration.getInteger("xxx");
    }

    @Test(expected = UnitilsException.class)
    public void notFoundWhenEmpty() {
        configuration.getInteger("empty");
    }

    @Test
    public void valueWithClassifiers() {
        Integer result = configuration.getInteger("propertyWithClassifiers", "a", "b");
        assertReflectionEquals(5, result);
    }
}
